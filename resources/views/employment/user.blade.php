...
<div class="mb-3">
    <div id="customer-table" class="mb-3">
        <a href="{{ route('employment-user.upload') }}" class="btn btn-primary mb-3">Upload Data</a>
        <div class="card">
                ...
                <table class="table table-bordered table-hover myTable"  id="tableEmployment">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>{{ $datas['eid'] }}</td>
                            <td>{{ $datas['name'] }}</td>
                            <td>@if ($datas['position']==1)
                                Senior Manager
                                @else 
                                Junior Manager
                            @endif</td>
                            
                            <td>@if ($datas['status']==1)
                                ACTIVE
                            ...
                            <td style="text-align: center; vertical-align:middle;">
                                <a class="btn btn-success" href="{{ route('employment-user.edit',['eid'=>$datas->eid]) }}">Update</a>
                                <a class="btn btn-info" href="{{ route('employment-info.user',['eid'=>$datas->eid])  }}">Info</a>
                            </td>
